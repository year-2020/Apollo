...
/**发布问题,还真是有点意思呢 */
  publish_question:function(e){
    ...
    app.api._fetch({//只需传title,content,imgUrl,latitude,longitude,address,tags）
      url: '/community/question/xcxadd',
      data: { 'title': that.data.titleText, 'content': that.data.contentText, 'imgUrl': imgsstr, 'tags': that.data.selectedTags.join(','), 'latitude': that.data.latitude, 'longitude': that.data.longitude, 'rewardPoints': that.data.xscredit, 'topicId': that.data.topicId},
      method: 'post'
    }).then(function (res) {
     if(res.data.code == 0){
  ...
  chooseImage: function (e) {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        console.log('照片数组!!!'+that.data.images[0])
        // 返回选定照片的本地文件路径列表，tempFilePath可以作为img标签的src属性显示图片  
        ...
        for (var i = 0, h = tempFilePaths.length; i < h; i++) {
          wx.uploadFile({
            url: 'https://apollodev.apollo.auto/community/upload/questionFile',
  ...
  uploadimg: function () {//这里触发图片上传的方法
    ...
    app.uploadimg({
      url: 'http://112.124.56.138:8080/apollo/community/upload/questionFile',//这里是你图片上传的接口
  ...
  onLoad: function (options) {
    ...
    console.info('topicId==' + options.name);
